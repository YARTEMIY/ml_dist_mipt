...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def rbf(x_1, x_2, sigma=1.):
        ...
        '''
        distances = torch.exp(-(torch.cdist(x_1, x_2) ** 2) / (2 * sigma ** 2))  ### YOUR CODE HERE
    ...
    def hinge_loss(scores, labels):
        ...
        return torch.mean(torch.maximum(torch.tensor(0.0), 1 - scores * labels))  ### YOUR CODE HERE
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return (torch.matmul(K, self.betas) + self.bias).flatten()  ### YOUR CODE HERE
